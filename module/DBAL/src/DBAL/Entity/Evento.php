...
class Evento extends \DBAL\Entity\ExtendedEntity {
    ...
    protected $rrpp;
     ...
     /**
     * @Annotation\Exclude()
     * @ORM\OneToMany(targetEntity="DBAL\Entity\Confirmado", mappedBy="evento")
     */
    ...
    {
        return $this->confirmados->toArray();
    }

    public function addConfirmados(\DBAL\Entity\Confirmado $confirmado)
    {
        if (!$this->confirmados->contains($confirmado)) {
            $this->confirmados->add($confirmado);
            $confirmado->setEvento($this);
        }

        return $this;
    }

    public function removeConfirmados(\DBAL\Entity\Confirmado $confirmado)
    {
        if ($this->confirmados->contains($confirmado)) {
            $this->confirmados->removeElement($confirmado);
            $confirmado->setEvento(null);
        }
    ...
    public function getContactos()
    {
        return array_map(
            function ($confirmado) {
                return $confirmado->getContacto();
            },
            $this->confirmados->toArray()
        );
    }
